...
def process_log_data(spark, input_data, output_data):
    ...
    df = df.withColumn("timestamp", get_timstamp(df.ts))
    
    # create datetime column from original timestamp column
    get_datetime = udf(lambda x: format_datetime(int(x)), DateTime())
    df = df.withColumn("datetime", get_datetime(df.ts))
    
    # extract columns to create time table
    time_table = spark.sql("""
                            SELECT DISTINCT timestamp as start_time, hour(timestamp) as hour, day(timestamp) as day, weekofyear(timestamp) as week, \
    ...
    songplays_table = spark.sql("""
                                SELECT DISTINCT sonplay_id, log.ts as start_time, log.userID as user_id, log.level, song.song_id, song.artist_id, \
                                                log.sessionId as session_id, log.location, log.user_agent
